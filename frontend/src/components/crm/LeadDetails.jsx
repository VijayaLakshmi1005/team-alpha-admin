...
export default function LeadDetails({ lead: initialLead, onClose, onGenerateInvoice }) {
  ...
  const handleUpdateNotes = async (notes) => {
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || ""}/api/leads/${lead._id}`, { notes });
  ...
  const handleSaveLead = async () => {
    setSaving(true);
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || ""}/api/leads/${lead._id}`, editData);
      setLead(response.data);
      setIsEditing(false);
  ...
  const sendReminder = async (member) => {
    // e.stopPropagation() handled in onClick
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || ""}/api/leads/${lead._id}/remind/${member}`);
