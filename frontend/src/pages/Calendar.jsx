...
export default function Calendar() {
    ...
    const handleSaveEvent = async (formData) => {
        try {
            if (selectedEvent) {
                // Update
                const res = await axios.patch(`${import.meta.env.VITE_API_URL || ""}/api/calendar/${selectedEvent._id}`, formData);
                setEvents(events.map(e => e._id === res.data._id ? res.data : e));
                toast.success("Event updated");
            } else {
                // Create
                const res = await axios.post(`${import.meta.env.VITE_API_URL || ""}/api/calendar`, formData);
    ...
    const handleDeleteEvent = async (id) => {
        ...
        try {
            await axios.delete(`${import.meta.env.VITE_API_URL || ""}/api/calendar/${id}`);
